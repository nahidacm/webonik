...
class Sm_Slideshow_Block_List extends Mage_Catalog_Block_Product_Abstract
{
	...
	protected function _toHtml(){
		...
		$template_file = "sm/slideshow/".$template.".phtml";
	...
	protected function _getItems(){
		...
		$collect_items = array();
		$this->addReviewSummaryTemplate('sm', 'sm/slideshow/summary.phtml');
		foreach( $products as $k => $_product ) {
			$item['id'] = $_product->getId();
			$item['title'] =  Mage::helper('slideshow/data')->truncate($_product->getName(), $this->_config['product_title_max_characters'], '...');
			...
			if (($maxchars=$this->getConfig('product_description_max_characters',-1))>0){
				$item['desc'] =  Mage::helper('slideshow/data')->truncate($item['desc'], $maxchars, '...');
			}
	...
	public function getScriptTags(){
		...
		if (null == Mage::registry('jsmart.jquery')){
			...
			if (Mage::getStoreConfigFlag('slideshow_cfg/advanced/include_jquery')){
				...
				$import_str .= $jsHelper->includeSkinScript('sm/slideshow/js/jquery-1.8.2.min.js');
				Mage::register('jsmart.jquery', 1);
			}
		}
		if (null == Mage::registry('jsmart.jquerynoconfict')){
			// add once noConflict
			$import_str .= $jsHelper->includeSkinScript('sm/slideshow/js/jquery-noconflict.js');
			Mage::register('jsmart.jquerynoconfict', 1);
		}
		if (null == Mage::registry('jsmart.slideshow.js')){
			// add script for this module.
			$import_str .= $jsHelper->includeSkinScript('sm/slideshow/js/jcarousel.js');
			Mage::register('jsmart.slideshow.js', 1);
		}
		return $import_str;
	}
